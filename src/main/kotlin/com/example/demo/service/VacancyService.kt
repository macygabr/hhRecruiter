...
@Repository
class VacancyService(private val webClient: WebClient.Builder) {

    private val applyVacanciesList = HashSet<String>()
    private val per_page = 100
    private val logger: Logger = LoggerFactory.getLogger(VacancyService::class.java)

    @Value("\${content.type}")
    private val contentType: String = ""

    @Value("\${negotiations.url}")
    private val negotiationsUrl: String = ""

    fun getInvitations(accessToken: String): HashSet<String> {
        val invitationsList = HashSet<String>()
        val response = createResponse("$negotiationsUrl?status=active", accessToken)

        response?.get("items")?.let { items ->
            (items as List<Map<String, Any>>).forEach { item ->
                val state = item["state"] as? Map<*, *>
                val stateId = state?.get("id") as? String
                if (stateId == "invitation") {
                    val vacancy = item["vacancy"] as? Map<*, *>
                    val alternateUrl = vacancy?.get("alternate_url") as? String
                    if (alternateUrl != null) {
                        invitationsList.add(alternateUrl)
                    }
                }
            }
        }
